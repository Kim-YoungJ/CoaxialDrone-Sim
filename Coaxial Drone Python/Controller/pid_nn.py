...
from NetworkPytorch import DNN_Func

def InitPID_NN(controller):
    # tau_D                   =   0.3
    # tau_velD                =   0.1
    ...
    controller.Igain_velD   =   (1 / tau_velD) * 0.3
    controller.Igain_p      =   (1 / tau_p) * 0.1
    controller.Igain_q      =   (1 / tau_q) * 0.1
    controller.Igain_r      =   (1 / tau_r) * 0.6
    # controller.Igain_r      =   (1 / tau_r) * 0.4
    ...
    try: 
        ...
        # model_file  =   '20231215-003453_4.pt'
        # model_file  =   '20231218-023112_23.pt'
        # model_file  =   '20231218-060205_34.pt'
        model_file  =   '20231218-084806_10.pt'
        os.chdir(model_path)
        Network = DNN_Func.DNN_Compensetor().to(dev)
    ...
    print("Network model file : " + model_file)
    controller.dev          =   dev
    
    controller.controller   =   "PID_NN"
...
def Allocator(controller, model, X, XCmd, Uprev, controlHz, dev):
    ...
    # Network compensate
    Inputdata       =   torch.cat([X[2,:], X[5,:], X[8,:], X[11,:],Uprev[1,:],Uprev[0,:]], dim=0).to(controller.dev)
    
    with torch.no_grad():
        controller.Network.eval()
        deltaU          =   controller.Network(Inputdata)
        
        FzControl       =   FzControl - deltaU[0]
        NControl        =   NControl  - deltaU[1]
