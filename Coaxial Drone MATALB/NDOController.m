function [U, InnerXcmd, FMb, Uad] = NDOController(xCmd, x, FMbprev, dt, param, controlParam)
...
zDot                =   zeros(2,1);
zDot(1)             =   -cZdot * (zdotNDO + g + (FMbprev(3)) * ((cos(theta)*cos(phi))/m) + cZdot*velNED(3));
zDot(2)             =   -cR * (rNDO - ((Ix-Iy)/Iz)*q*p + FMbprev(6)/Iz + cR * r);

zdotNDO             =   zdotNDO + zDot(1)*dt;
rNDO                =   rNDO + zDot(2)*dt;

NDO(1)              =   zdotNDO + cZdot*velNED(3);
NDO(2)              =   rNDO + cR*r;
...
FzControl           =   (m / (cos(theta)*cos(phi))) * (K_velD * (errorVelD) + Izdot) + Fz_trim -(m / (cos(theta)*cos(phi)))*NDO(1);
...
NControl            =   Iz * ((K_r * errorR) + Ir) + N_trim - Iz*NDO(2);
...
FMb                 =   [0; 0; FzControl; LControl; MControl; NControl];
Uad                 =   NDO;
